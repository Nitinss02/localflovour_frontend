...
function VendorSignUpPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    if (name.trim() === "") { valid = false; toast.error("Business Name is required."); }
    ...
    >
      ...
      <div className="relative z-10 w-full max-w-md py-8 px-6 sm:px-10 p-2 rounded-2xl shadow-2xl bg-black/bg-opacity-30">
        ...
        <div className="text-center mb-6">
          <h2 className="text-2xl font-bold text-gray-100">Vendor Registration</h2>
          <p className="text-sm text-gray-100 mt-2">
            Join as a vendor and start adding your products today!
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-y-3 text-sm">
          ...
          <Button
            ...
            disabled={isLoading}
          >
            {isLoading ? "Signing Up..." : "Register as Vendor"}
          </Button>

          <p className="text-center text-xs text-gray-200 mt-3">
            Already registered as a vendor?
            <Link to="/vendor/login" className="font-medium text-indigo-600 hover:text-indigo-500 ml-1">
              Sign In
            </Link>
          </p>
        </form>
